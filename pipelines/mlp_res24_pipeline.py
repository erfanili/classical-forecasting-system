...
def reconstruct_load_from_res24(pred_res24, df_load, last_timestamp, steps):
    ...
    idx = df_load.index.get_loc(last_timestamp)
    last_24_actual = df_load.iloc[idx-23 : idx+1].values
...
def main(args):
    ...
    df['res24'] = df['load_mw'] - df['load_mw'].shift(24)
    ...
    os.makedirs('results/mlp_res24',exist_ok=True)
    with open(f'results/mlp_res24/test_res_{args.window}_{test_start}.json','w') as f:
        json.dump(results,f)
    # ---------------- Plot ----------------
    ...
    plt.savefig(f'results/mlp_res24/test_res_{args.window}_{test_start}.jpg')
